...
#[test]
#[serial]
fn test_alpenglow_equivocation_basic() {
    agave_logger::setup_with_default(AG_DEBUG_LOG_FILTER);
    // Create node stakes
    let slots_per_epoch = 512;
    ...
    a_validator_config.repair_handler_type = RepairHandlerType::Malicious(MaliciousRepairConfig {
        ...
        bad_shred_index_frequency: Some(32),
        per_requester_equivocation: false,
    });
...
/// We start 3 nodes: A (20%), B (40%), C (40%).
/// B and C have turbine disabled, so they rely on repair.
/// A produces equivocating blocks where each requester gets a different consistent block.
/// This means B and C will each receive valid blocks, but they'll be different.
/// Through informed repair (using block_id), B and C should be able to converge
/// to the same block that has votes from the other node.
#[test]
#[serial]
fn test_alpenglow_equivocation_full_3_versions() {
    agave_logger::setup_with_default(AG_DEBUG_LOG_FILTER);

    let slots_per_epoch = 512;

    // Create node stakes: A=20%, B=40%, C=40%
    let total_stake = 5 * DEFAULT_NODE_STAKE;
    let node_a_stake = total_stake / 5; // 20%
    let node_b_stake = 2 * total_stake / 5; // 40%
    let node_c_stake = total_stake - node_a_stake - node_b_stake; // 40%

    let node_stakes = vec![node_a_stake, node_b_stake, node_c_stake];

    // Create leader schedule with A as the only leader
    let (leader_schedule, validator_keys) =
        create_custom_leader_schedule_with_random_keys(&[4, 0, 0]);
    ...
    // Node A config - the malicious leader
    let mut a_validator_config = ValidatorConfig::default_for_test();
    a_validator_config.wait_for_supermajority = Some(0);
    a_validator_config.fixed_leader_schedule = Some(leader_schedule);
    a_validator_config.voting_service_test_override = Some(VotingServiceOverride {
    ...
    // Configure A to send different blocks to different requesters
    a_validator_config.repair_handler_type = RepairHandlerType::Malicious(MaliciousRepairConfig {
        bad_shred_slot_frequency: Some(10),
        bad_shred_index_frequency: None,
        per_requester_equivocation: true,
    });

    // Node B config - turbine disabled
    let mut b_validator_config = safe_clone_config(&a_validator_config);
    b_validator_config
        .turbine_disabled
        .store(true, Ordering::Release);
    // B should not be malicious
    b_validator_config.repair_handler_type = RepairHandlerType::Standard;

    // Node C config - turbine disabled
    let mut c_validator_config = safe_clone_config(&a_validator_config);
    c_validator_config
        .turbine_disabled
        .store(true, Ordering::Release);
    // C should not be malicious
    c_validator_config.repair_handler_type = RepairHandlerType::Standard;

    // Cluster config
    let mut cluster_config = ClusterConfig {
        mint_lamports: total_stake,
        node_stakes: node_stakes.clone(),
        validator_configs: vec![a_validator_config, b_validator_config, c_validator_config],
        validator_keys: Some(
            validator_keys
                .iter()
                .cloned()
    ...
    // Ensure all nodes are rooting - need more roots to verify B and C converge
    cluster.check_for_new_roots(
        32,
        "test_alpenglow_targeted_equivocation",
        SocketAddrSpace::Unspecified,
    );
}
