...
impl VotingLoop {
    ...
    fn voting_loop(config: VotingLoopConfig) {
        ...
        let mut current_slot = {
            ...
            let slot = bank_forks.read().unwrap().working_bank().slot() + 1;
            info!(
        ...
        loop {
            ...
            if is_leader {
                ...
                Self::notify_block_creation_loop_of_leader_window(
                    ...
                    start_slot,
                    leader_end_slot,
                    (start_slot - 1, Hash::default(), Hash::default()),
                    skip_timer,
                );
            }
            ...
            while current_slot <= leader_end_slot {
                ...
                info!(
                    "{my_pubkey}: Slot {current_slot} observed in {} ms. Skip timer {} vs timeout {}",
